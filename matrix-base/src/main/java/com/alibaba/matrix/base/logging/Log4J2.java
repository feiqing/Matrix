...
public class Log4J2 {
    ...
    public void loadConfiguration(String configFile) {
        ...
        Configuration newConfiguration = loadConfiguration(context, configFile);
    ...
    private Configuration loadConfiguration(LoggerContext context, String location) {
        try {
            URL url = MatrixResourceUtils.getResourceUrl(location);
    ...
    private ConfigurationSource getConfigurationSource(URL url) throws IOException {
        ...
        if (FILE_PROTOCOL.equals(url.getProtocol())) {
            return new ConfigurationSource(stream, MatrixResourceUtils.getResourceAsFile(url));
        } else {
